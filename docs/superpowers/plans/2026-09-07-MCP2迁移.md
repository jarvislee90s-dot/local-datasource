...
- [x] **Step 2: 记录基线**（写入执行日志）：`python -m pytest tests/ -q` 的通过数与耗时；`git rev-parse HEAD`。

- [x] **Step 3: 升级依赖声明**——`pyproject.toml` 第 7 行：
...
- [x] **Step 5: 旧代码在新库下全量测试仍绿**（`build_tools()` 只构造 `mcp.types.Tool`，不触 `_main()` 装饰器，预期不受影响）
...
- [x] **Step 1: 全量替换 `tests/test_server.py`**
...
- [x] **Step 2: 修改 `tests/test_contract_regression.py` 入口取数**——顶部 import 区：
...
- [x] **Step 1: 写入新 `server.py`**。骨架如下（docstring 的 16 工具清单沿用基线文件头；`query_yfinance` 的 config 回退逻辑在分发层实现）：
...
- [x] **Step 2: 按转录表补齐其余 12 个工具函数**。规则：**参数名、枚举值、默认值、required 与下表严格一致；`Field(description=…)` 与工具 docstring 一律逐字取自基线 `build_tools()` 对应字符串（`git show a4d2258:src/local_datasource/server.py`）**；函数体一行 `_safe_summary("<名>", {<全部参数名逐个入 dict>})`。
...
- [x] **Step 3: 默认值对齐核查**——逐一对照 provider 函数签名（如 `query_stock(..., adjust="qfq", ...)`），确认薄函数默认值与 provider 默认值一致；发现不一致时**以 provider 默认值为准**改薄函数并记入偏差项（防止"缺省传参"从 provider 默认漂移为 schema 默认）。

- [x] **Step 4: 导入冒烟 + schema 冒烟**
...
- [x] **Step 5: 描述保真抽查**——写临时脚本（不入库）对比 3 个工具（建议 query_futures/query_spot/query_bond）的 `t.description` 与基线字符串逐字一致，参数抽查 2 个 `Field` description。不一致即为转录笔误，修到一致。

- [x] **Step 6: 跑 Task 1 改的两个测试文件 + 全量**
...
- [x] **Step 1: 写入 `tests/test_stdio_probe.py`**
...
- [x] **Step 1: CHANGELOG 新增条目**（按文件现有格式插入今日条目；措辞可微调，事实不得超出）：
...
- [x] **Step 4: spec §6 逐条对照**——在执行日志中逐项列出 8 条验收标准与其证据（命令+结果）。其中"任选 3 个工具 tools/call"一条如实说明覆盖边界：纯本地（离线可稳定回归的）工具只有 2 个，探针以 3 次成功调用（query_trading_rules 两种参数 + align_series 端到端）覆盖输出格式契约，其余 14 个工具与 1.x 走完全相同的 provider 层（由既有测试覆盖）；此为有意取舍，写入日志偏差项。
