...
- `query_bond(kind=issue_info)` 按发行人/按代码查恢复可用。断裂根因（2026-09 实测，akshare ≤1.18.94 均未修复）：货币网改版后列表接口 `BondMarketInfoList2` 强制要求 `bondType`，不带时返回错误结构（无 `pageTotal` 键）导致 akshare `KeyError`；其文档指向的查询页已 403 下线
- `providers/bond.py` 新增货币网直连：取类型字典（30 个债券类型）后遍历查询合并，输出列与原 akshare 完全一致（7 个中文列）；输出行为不变——按代码精确过滤、按发行人返回最新一只

### 变更

- 直连适配器尊重站点限流（三出口实证：WAF 按"IP × 窗口请求数"限，阈值个位数请求，pageSize>15 直接 403）：**全串行 + 进程级单连接 keep-alive + 每请求节流 2.5s**；421 时静默退避 4 分钟自动重试一次，仍限流则中止并如实提示；任一类型重试后仍失败则整体报错并列出失败类型，绝不静默返回缺类型的残缺结果
- 实测注意：单次"按发行人查"约 75 秒（30 类型节流遍历）；同一会话内重复查同一发行人建议走 `download` 缓存避免重复触发限流
