...
- 直连适配器尊重站点限流：共享会话 keep-alive（30 个类型请求仅复用 2 条持久连接）+ 并发 2 路；触发连接数限流（HTTP 421，按 IP 限新建连接数）时立即中止剩余请求并提示稍候重试；任一类型重试后仍失败则整体报错并列出失败类型，绝不静默返回缺类型的残缺结果
- 实测注意：单次"按发行人查"约 6 秒（30 类型 × 2 路并发）；同一会话内重复查同一发行人建议走 `download` 缓存避免重复触发限流
